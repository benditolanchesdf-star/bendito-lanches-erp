                  ...
                  <Link href={item.href} onClick={() => setIsOpen(false)}
                    className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all relative
                      ${isActive ? 'bg-bendito-dourado text-bendito-verde-escuro font-semibold'
                                  : hasAlerta
                                    ? 'bg-yellow-400/95 text-yellow-950 font-semibold animate-pulse hover:bg-yellow-300'
                                    : 'hover:bg-bendito-verde text-bendito-creme'}`}>
                    <Icon size={20} />
                    <span className="flex-1">{item.label}</span>
                    {item.badge !== undefined && item.badge > 0 && (
                      <span className={`text-xs font-bold rounded-full min-w-[1.5rem] px-1.5 py-0.5 text-center
                        ${hasAlerta ? 'bg-red-600 text-white' : 'bg-bendito-dourado text-bendito-verde-escuro'}`}>
