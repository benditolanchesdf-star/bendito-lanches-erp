...
export default function AtendentesPage() {
  ...
  async function load() {
    ...
    const [{ data: at }, { data: fils }] = await Promise.all([
      supabase.from('atendentes_pdv').select(`*, filiais(nome), atendente_filiais(filial_id, ativo, filiais(nome))`).order('nome'),
  ...
  async function salvar() {
    ...
    setSalvando(true)
    if (editando) {
      await supabase.from('atendentes_pdv').update({ nome: form.nome, filial_id: form.filial_id }).eq('id', editando.id)
    } else {
  ...
  async function abrirFiliais(a: any) {
    setAtendenteSel(a)
    const ativas = (a.atendente_filiais || []).filter((af: any) => af.ativo).map((af: any) => af.filial_id)
  ...
  async function salvarFiliais() {
    ...
    if (filiaisAtendente.length > 0) {
      await supabase.from('atendente_filiais').insert(
        filiaisAtendente.map(fid => ({ atendente_id: atendenteSel.id, filial_id: fid, ativo: true }))
      )
    }
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Atendentes PDV" subtitle="Frente de caixa — senha inicial: 1234"
        action={
          <PrimaryButton onClick={() => { setEditando(null); setForm({ nome: '', filial_id: FILIAL_ID }); setModalOpen(true) }}
            className="flex items-center gap-2"><Plus size={16} /> Novo atendente</PrimaryButton>
        }
      />

      <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-xs text-blue-700">
        💡 Senha inicial <strong>1234</strong>. O atendente define senha pessoal no primeiro acesso ao PDV.
        Use <strong>Resetar</strong> para voltar para 1234. Use <strong>Unidades</strong> para definir filiais autorizadas.
      </div>
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>{['Nome','Unidades','1º Acesso','Status','Ações'].map(h =>
                  ...
                  <tr key={a.id} className="hover:bg-gray-50">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {filiaisAtivas.length === 0
                          ...
                          : filiaisAtivas.map((af: any) => (
                            <span key={af.filial_id} className="px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">
                              {af.filiais?.nome}
                            </span>
                          ))
                        }
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${a.primeiro_acesso ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>
                        {a.primeiro_acesso ? 'Pendente' : 'Definida'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        <button onClick={() => { setEditando(a); setForm({ nome: a.nome, filial_id: a.filial_id }); setModalOpen(true) }}
                          title="Editar" className="p-1.5 text-gray-400 hover:text-bendito-verde rounded"><Edit size={14}/></button>
                        <button onClick={() => abrirFiliais(a)} title="Unidades"
                          className="p-1.5 text-gray-400 hover:text-blue-600 rounded"><Building2 size={14}/></button>
                        <button onClick={() => resetarSenha(a)} title="Resetar senha"
        ...
        <div className="space-y-4">
          ...
          {!editando && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-xs text-yellow-700">
              🔑 Senha inicial: <strong>1234</strong>. O atendente deverá alterá-la no primeiro acesso ao PDV.
            </div>
          )}
      ...
      <Modal isOpen={modalFiliaisOpen} onClose={() => setModalFiliaisOpen(false)} title={`Unidades — ${atendenteSel?.nome}`}>
        <div className="space-y-4">
          <p className="text-sm text-gray-600">Selecione as unidades onde este atendente pode operar.</p>
          <div className="space-y-2">
            {filiais.map(f => {
              const ativo = filiaisAtendente.includes(f.id)
              return (
                <button key={f.id} onClick={() => setFiliaisAtendente(prev =>
