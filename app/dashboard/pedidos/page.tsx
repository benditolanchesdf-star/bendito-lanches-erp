...
export default function PedidosPage() {
  ...
  async function load() {
    ...
    const { data } = await supabase
      .from('pedidos')
      .select('id, numero_pedido, canal, status, valor_total, created_at, data_entrega, horario_entrega, clientes(nome, nome_loja, telefone)')
  ...
  async function mudarStatus(id: string, novo: string) {
    ...
    // Dispara notificação WhatsApp (assíncrono, não bloqueia UI)
    const pedido = pedidos.find((p) => p.id === id)
    if (pedido) {
      notificarStatusPedido({ ...pedido, status: novo }).catch(() => {})
    }
  }
