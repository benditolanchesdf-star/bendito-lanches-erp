    ...
    try {
      ...
      const [cli, ped, com, alt] = await Promise.all([
        ...
        supabase.from('pedidos')
          .select('id, numero_pedido, status, valor_total, created_at, clientes(nome, nome_loja)')
          .gte('created_at', inicioMes)
          .neq('status', 'cancelado')
          .order('created_at', { ascending: false })
          .limit(20),
        supabase.from('comissoes').select('valor_comissao, status').eq('status', 'pendente'),
        supabase.from('vw_clientes_alertas')
          .select('*')
      ...
      console.error('Erro ao carregar painel vendedor:', err)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Sua carteira de clientes em tempo real"
        action={
          <Link href="/vendedor/pedidos/novo">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          ...
          {alertas.length === 0
            ? <EmptyState message="Nenhum cliente com alerta." />
            : (
                ...
                {alertas.slice(0, 10).map((a) => {
                  ...
                  return (
                    <div key={a.cliente_id} className="flex justify-between items-center py-2 border-b last:border-0">
                      ...
                      {meta && (
                        <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${meta.cor}`}>
                          {meta.label}
                        </span>
                      )}
                    </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          ...
          {pedidosRecentes.length === 0
            ? <EmptyState message="Nenhum pedido este mês." />
            : (
              <div className="space-y-2">
                    ...
                    <div key={p.id} className="flex justify-between items-center py-2 border-b last:border-0">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-gray-500 truncate">
                          {(p.clientes as any)?.nome_loja || (p.clientes as any)?.nome}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        {st && (
                          <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${st.cor}`}>
                            {st.label}
                          </span>
                        )}
                        <span className="text-sm font-bold text-bendito-verde">
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-5">
      <div className="flex items-center justify-between">
        ...
        <div className={`${cor} p-3 rounded-full shrink-0`}>
